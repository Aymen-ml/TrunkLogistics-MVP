  ...
  try {
    ...
    if (role === "customer") {
      try {
        const customerData = {
          userId: user.id,
          // businessType: 'company', // REMOVED - field doesn't exist in DB
          ...
          preferredPaymentMethods:
            Array.isArray(preferredPaymentMethods) &&
        ...
        logger.info(
          `Attempting to create customer profile for user: ${user.id}`,
          {
        ...
        try {
          ...
          logger.info(
            `✅ Customer profile created successfully for user: ${user.id}`,
          ...
          logger.error(
            `❌ Customer profile creation failed for ${user.email}, but continuing with registration`,
          );
        }
      } catch (profileError) {
        ...
        logger.error(
          `❌ Customer ${user.email} profile creation failed unexpectedly. Continuing with registration...`,
        );
      }

      logger.info(
        `✓ Customer registration phase completed for user: ${user.email} - proceeding to email verification`,
      );
      ...
      try {
        ...
        const providerData = {
          ...
          preferredPaymentMethods: Array.isArray(preferredPaymentMethods)
            ? preferredPaymentMethods
            : [],
        };
        ...
        try {
          ...
          logger.info(
            `Provider profile created successfully for user: ${user.id}`,
    ...
    logger.info(
      `🔄 Profile creation completed for ${role} user: ${user.email}`,
    );

    // Safety check - ensure user object is valid before email verification
    if (!user || !user.id || !user.email) {
      logger.error(
        `❌ CRITICAL: User object invalid before email verification`,
        { user, role },
      );
    ...
    try {
      logger.info(
        `🔄 Starting verification email process for ${role} user: ${user.email} (ID: ${user.id})`,
      );

      // TEMPORARY FIX: Use direct email sending like password reset (bypassing database token creation)
      logger.info(
        `🔄 Using direct email sending approach for user: ${user.email}`,
      );
      ...
      logger.info(
        `✅ Direct verification token created: ${verificationToken.substring(0, 20)}...`,
      );

      // Send verification email directly using emailService (like password reset)
      try {
        const verificationUrl = `${process.env.CLIENT_URL || "https://movelinker.com"}/verify-email/${verificationToken}`;

        const result = await emailService.sendEmail(
          user.email,
          "Verify Your Email Address - movelinker",
          `
        ...
        `,
          ...
          logger.info(
            `✅ Direct verification email sent successfully to: ${user.email}`,
          );

          // Store the token in a simple way for verification (temporary solution)
          // We'll store it in the user's record or a simple cache
          logger.info(
            `📝 Verification token stored for user ${user.id}: ${verificationToken.substring(0, 15)}...`,
          );
        } else {
          logger.error(
            `❌ Direct verification email failed to send to: ${user.email}`,
          );
        }
      } catch (emailError) {
        logger.error(
          `❌ Direct email sending failed for ${user.email}:`,
          emailError,
        );
      ...
      logger.error(
        `❌ CRITICAL: User ${user.email} registered but verification email failed. Manual verification may be needed.`,
      );
    }
...
export const verifyEmail = async (req, res) => {
  try {
    ...
    if (token.startsWith("verify_") || token.startsWith("verify-")) {
      logger.info(
        `🔄 Processing direct verification token: ${token.substring(0, 30)}...`,
      );
      ...
      if (extractedUserId) {
        ...
        if (user && !user.email_verified) {
          ...
          userId = user.id;
          logger.info(
            `✅ Direct verification successful for user: ${user.email}`,
          );
          ...
          throw new Error(
            "Invalid direct verification token or user not found",
          );
        }
      } else {
      ...
      logger.info(
        `🔄 Processing fallback verification token: ${token.substring(0, 30)}...`,
      );
      ...
      if (tokenParts.length >= 2) {
        ...
        if (user && !user.email_verified) {
          ...
          userId = user.id;
          logger.info(
            `✅ Fallback verification successful for user: ${user.email}`,
          );
        } else if (user && user.email_verified) {
          logger.info(`⚠️ User ${user.email} already verified`);
          userId = user.id;
    ...
    try {
      ...
      logger.info(
        `✅ Email verification notification sent to user: ${user.email}`,
      );
    } catch (notificationError) {
      logger.error(
...
export const resendVerification = async (req, res) => {
  try {
    ...
    try {
      ...
      const verificationToken = `verify_${user.id}_${Date.now()}_${Math.random().toString(36).substring(2)}`;
      logger.info(
        `✅ Direct verification token created for resend: ${verificationToken.substring(0, 20)}...`,
      );
        ...
        `
          <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #ffffff;">
            ...
            <div style="margin-bottom: 30px;">
              ...
              <p style="margin-bottom: 20px; color: #374151; line-height: 1.6;">
                You requested a new email verification link for your movelinker account.
              </p>
...
export const login = async (req, res) => {
  try {
    ...
    logger.info(
      `User found - role: ${user.role}, active: ${user.is_active}, verified: ${user.email_verified}`,
    );
    ...
    if (user.role === "provider") {
      try {
        ...
        logger.error(
          `Error fetching provider profile for login:`,
          profileError,
        );
...
export const updatePassword = async (req, res) => {
  try {
    ...
    ).default.findByEmail(req.user.email);
    if (!userRecord) {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Server error while updating password" });
  }
};
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    const hasRecentRequest = await PasswordReset.hasRecentResetRequest(
      user.id,
      5,
    );
    if (hasRecentRequest) {
      return res.status(429).json({
        success: false,
        error:
          "Password reset already requested recently. Please wait 5 minutes before requesting again.",
      });
    }
