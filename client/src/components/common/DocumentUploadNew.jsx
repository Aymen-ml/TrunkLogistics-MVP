...
const DocumentUploadNew = ({
  ...
  // Remove document
  const removeDocument = useCallback((value, onChange) => {
    if (value && !value.isExisting && value.preview) {
  ...
  const removeAdditionalDocument = useCallback((index) => {
    const currentDocs = Array.isArray(additionalDocs) ? additionalDocs : [];
    const docToRemove = currentDocs[index];
    
    if (docToRemove && !docToRemove.isExisting && docToRemove.preview) {
  ...
  }, [additionalDocs, onAdditionalDocsChange]);
