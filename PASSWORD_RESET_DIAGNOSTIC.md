...
const testEmail = async () => {
  try {
    const result = await emailService.sendPasswordResetEmail(
      'test@example.com',
      'Test User',
      'https://movelinker.com/reset-password/test-token'
    );
...
const API_URL = import.meta.env.VITE_API_URL || 'https://api.movelinker.com/api';
```
...
```bash
curl -X POST https://api.movelinker.com/api/auth/forgot-password \
...
4. **Reset Password:**
```bash
curl -X POST https://api.movelinker.com/api/auth/reset-password/[TOKEN] \
  -H "Content-Type: application/json" \
